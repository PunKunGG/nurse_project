 ...
 * Run this once to migrate hardcoded questions to Supabase
 *
 * HOW TO USE:
 * 1. Open browser console on any page of the app (after login)
 * 2. Import and run:
 *    import('./assets/js/migrateQuestions.js').then(m => m.migrateQuestions())
 */

import { supabase } from "./supabaseClient.js";

// Pre-test Questions (from pretestQuiz.js)
...
// Post-test Questions (same as pretest for now)
const posttestQuestions = pretestQuestions;

/**
 * Migrate all questions to Supabase
 */
export async function migrateQuestions() {
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      console.log(
        "⚠️ Questions already exist in database. Skipping migration.",
      );
      console.log("   If you want to re-migrate, delete all questions first.");
      return;
    }
    ...
    console.log(`✅ Successfully migrated ${data.length} questions!`);
    console.log("   - Pre-test questions:", pretestQuestions.length);
    console.log("   - Post-test questions:", posttestQuestions.length);
  } catch (error) {
...
window.migrateQuestions = migrateQuestions;

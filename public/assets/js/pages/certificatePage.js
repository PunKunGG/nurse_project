...
if (aErr || !attempts || attempts.length === 0) {
  ...
  if (canDownload) {
    ...
    scoreDetail.textContent = `ผ่านเกณฑ์ ≥ 80% ✓`;
    ...
    statusSubtitle.textContent = `ต้องได้ ≥ 80% แต่คุณได้ ${latestScore}%`;
    scoreDetail.textContent = `ต้องได้อีก ${80 - latestScore}% จึงจะผ่าน`;
    downloadBtn.disabled = true;
